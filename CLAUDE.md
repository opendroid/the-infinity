...
| `cd web && npm run find:videos -- search` | **Runs on a laptop, not here** — finds candidate `kind: video` explainers through the YouTube Data API ([#384](https://github.com/opendroid/the-infinity/issues/384)). Needs `YOUTUBE_API_KEY`; the free tier is 100 searches/day, so it checkpoints and resumes. Searches the 117 domains by default, `--concepts` for all 482. It emits candidates for a human to pick — it never writes `content/nodes/**`. `verify picks.json` then turns picks into entries whose title and author are oEmbed's own strings, so `check:explainers` agrees with them by construction |
