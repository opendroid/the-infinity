...
| `cd api && make analytics` | What readers did, from the Firebase Hosting request log — top concepts, crawler share, the cache-hit ratio that is the static-first claim measured — reported **separately for pages and for hashed assets**, because `firebase.json` caches them to opposite policies and one blended number answers neither ([#427](https://github.com/opendroid/the-infinity/issues/427)) — and concept-to-concept moves split two ways: **edges pulled** (a reader followed a declared edge) and **jumped, no edge** — pairs the readership connected and the graph does not, which are candidate edges ([#426](https://github.com/opendroid/the-infinity/issues/426)). Telling them apart needs the node corpus, so it reads `-nodes` (default `../content/nodes`) through the same `internal/publish` derivation the API publishes. Read-only, and no analytics JavaScript ships to anybody ([ADR-0011](docs/adr/0011-analytics-from-request-logs.md)). Needs Cloud Logging linked and credentials. `ANALYTICS_DAYS=30 make analytics`, or `go run ./cmd/analytics -top 25`; `-days` is capped at 30 because that is the log bucket's retention |
